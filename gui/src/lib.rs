...
use three_d::{
    ...
    ColorMaterial, Cull, DirectionalLight, Event, FrameOutput, FreeOrbitControl, Gm, InnerSpace,
    InstancedMesh, Instances, Light, Mat4, Mesh, MouseButton, Object, PhysicalMaterial, PointLight,
    RendererError, Srgba, TextGenerator, TextLayoutOptions, Vec3, Vec4, Viewport, Window,
    WindowSettings,
};
...
const COLOR_FIERY_GREEN: Srgba = Srgba::new_opaque(0, 255, 80);
const COLOR_FIERY_BLUE: Srgba = Srgba::new_opaque(80, 0, 255);
...
fn generate_glyphs(
    ...
    font: &[u8],
    font_size: f32,
) -> Vec<(Gm<Mesh, ColorMaterial>, usize, Mat4)> {
...
struct UIState {
    ...
    has_changes: bool,
    unscrambling: bool,
    unscrambling_one: bool,
    scrambling: bool,
    scrambling_one: bool,
    scrambling_count: usize,
}
...
impl UIState {
    fn new() -> Self {
        UIState {
            ...
            unscrambling: false,
            scrambling: false,
            unscrambling_one: false,
            scrambling_one: false,
            scrambling_count: 0,
        }
    }
    ...
    fn reset(&mut self) {
        ...
        self.unscrambling = false;
        self.unscrambling_one = false;
        self.scrambling = false;
        self.scrambling_one = false;
        self.scrambling_count = 0;
    }
...
fn run(mut model: Model) {
    ...
    fn facet_colors_init() -> Vec<Srgba> {
        let colors = [
            ...
            Srgba::new_opaque(127, 127, 127), //13
    ...
    let mut directional_light_r =
        DirectionalLight::new(&context, 0.7, COLOR_FIERY_RED, vec3(-0.5, -0.86, -1.0));
    ...
    window.render_loop(move |mut frame_input| {
        ...
        gui.update(
            ...
            |gui_context| {
                ...
                SidePanel::left("side_panel").show(gui_context, |ui| {
                    ...
                    if ui
                        .add_enabled(
                            !ui_state.unscrambling && !ui_state.scrambling,
                            Button::new("Scramble"),
                        )
                        .clicked()
                    {
                        ui_state.scrambling = true;
                        ui_state.scrambling_count = 6;
                        ui_state.picked_facet_id = None;
                    ...
                    if ui
                        .add_enabled(
                            !ui_state.unscrambling && !ui_state.scrambling,
                            Button::new("Unscramble"),
                        )
                        .clicked()
                    {
                        ui_state.unscrambling = !model.moves.is_empty();
                        ui_state.picked_facet_id = None;
                        ui_state.has_changes = true;
                    }
        ...
        if ui_state.unscrambling {
            if !ui_state.unscrambling_one {
                if let Some((i, clockwise)) = model.moves.pop() {
                    for (j, rot) in ui_state.rotating.iter_mut().enumerate() {
                        if i == j {
                            *rot = Some((0., !clockwise));
                            ui_state.unscrambling_one = true;
                        } else {
                            *rot = None;
                        }
                    }
                    ...
                    // not required atm
                    ui_state.unscrambling = false;
                    ui_state.unscrambling_one = false;
                }
            }
        } else if ui_state.scrambling {
            if !ui_state.scrambling_one && ui_state.scrambling_count > 0 {
                let i = model.rng.next_u32() as usize % 12;
                let clockwise = model.rng.next_u32() % 2 == 0;
                for (j, rot) in ui_state.rotating.iter_mut().enumerate() {
                    if i == j {
                        *rot = Some((0., clockwise));
                        ui_state.scrambling_one = true;
                    } else {
                        *rot = None;
                    }
                }
                    ...
                    *rot_opt = if next_rot == PENTA_ANGLE {
                    ...
                    if next_rot == PENTA_ANGLE {
                        ...
                        if ui_state.unscrambling {
                            ...
                            model.moves.push((i, clockwise));
                            if ui_state.scrambling {
                                ui_state.scrambling_count -= 1;
                                ui_state.scrambling_one = false;
                                if ui_state.scrambling_count == 0 {
                                    ui_state.scrambling = false;
                                }
                            }
                        }
        ...
        if ui_state.has_changes {
            ...
            if let Some(id) = ui_state.picked_facet_id {
                ...
                for num in glyphs.iter_mut().skip(id * 5).take(5) {
                    num.0.material.color = Srgba::WHITE;
                }
            }
        ...
        let cam_pos = camera.position().normalize();
        let up = camera.up();
        point_light.position = 3. * cam_pos;
        directional_light_r.direction = -cam_pos - up;

        let lights = [&ambient as &dyn Light, &point_light, &directional_light_r];
